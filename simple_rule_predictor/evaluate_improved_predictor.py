# evaluate_improved_predictor.py
import sys
import os
...
import pandas as pd
import numpy as np
from simple_rule_predictor_improved import calculate_improved_prediction
from config_improved import DATA_PATH

def evaluate_improved_predictor():
    """评估改进版规则预测器的准确率"""
    ...
    for i in range(start_idx, end_idx):
        ...
        try:
            # 预测下一根K线（静默模式）
            result = calculate_improved_prediction(df_subset, silent=True)
    ...
    if total_predictions > 0:
        ...
        if total_weighted > 0:
            ...
            print(f"加权平均准确率: {weighted_acc:.2f}%")
        
    else:
        print("没有有效的预测结果")

if __name__ == "__main__":
    evaluate_improved_predictor() 