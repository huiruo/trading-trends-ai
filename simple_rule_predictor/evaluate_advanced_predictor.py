# evaluate_advanced_predictor.py
import sys
import os
...
def evaluate_advanced_predictor():
    """评估高级规则预测器的准确率"""
    ...
    for i in range(start_idx, end_idx):
        ...
        try:
            # 预测下一根K线（静默模式）
            result = calculate_advanced_prediction(df_subset, silent=True)
            ...
            # 按市场情绪统计
            if market_sentiment not in sentiment_stats:
                sentiment_stats[market_sentiment] = {'correct': 0, 'total': 0}
            sentiment_stats[market_sentiment]['total'] += 1
            if pred_direction == actual_direction:
                sentiment_stats[market_sentiment]['correct'] += 1
    ...
    if total_predictions > 0:
        ...
        for sentiment, stats in sentiment_stats.items():
            if stats['total'] > 0:
                ...
                print(f"{sentiment_name}: {acc:.2f}% ({stats['correct']}/{stats['total']})")
